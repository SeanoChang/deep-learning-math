...
export default async function ProbabilityTwoLessonPage({ params }: PageProps) {
  const { slug } = await params;
  const lesson = getLessonContent("probability-2", slug);
  ...
  const adjacent = getAdjacentLessons("probability-2", slug);
  ...
  return (
    <div className="flex gap-10">
      <article className="prose flex-1 min-w-0">
        <Breadcrumb
          items={[
            { label: "Probability II", href: "/probability-2" },
        ...
        {content}
        <LessonNav topicId="probability-2" prev={adjacent.prev} next={adjacent.next} />
      </article>
      <TableOfContents />
    </div>
  );
