...
export async function generateStaticParams() {
  const lessons = getTopicLessons("linear-algebra-2");
  return lessons.map((l) => ({ slug: l.slug }));
}

export default async function LinearAlgebraTwoLessonPage({ params }: PageProps) {
  const { slug } = await params;
  const lesson = getLessonContent("linear-algebra-2", slug);
  ...
  const adjacent = getAdjacentLessons("linear-algebra-2", slug);
  ...
  return (
    <div className="flex gap-10">
      <article className="prose flex-1 min-w-0">
        <Breadcrumb
          items={[
            { label: "Linear Algebra II", href: "/linear-algebra-2" },
        ...
        {content}
        <LessonNav topicId="linear-algebra-2" prev={adjacent.prev} next={adjacent.next} />
      </article>
      <TableOfContents />
    </div>
  );
