...
export async function generateStaticParams() {
  const lessons = getTopicLessons("deep-learning");
  return lessons.map((l) => ({ slug: l.slug }));
}

export default async function DeepLearningLessonPage({ params }: PageProps) {
  const { slug } = await params;
  const lesson = getLessonContent("deep-learning", slug);
  ...
  const adjacent = getAdjacentLessons("deep-learning", slug);
  ...
  return (
    <div className="flex gap-10">
      <article className="prose flex-1 min-w-0">
        <Breadcrumb
          items={[
            { label: "Deep Learning Foundations", href: "/deep-learning" },
        ...
        {content}
        <LessonNav topicId="deep-learning" prev={adjacent.prev} next={adjacent.next} />
      </article>
      <TableOfContents />
    </div>
  );
