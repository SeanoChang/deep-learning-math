...
export async function generateStaticParams() {
  const lessons = getTopicLessons("calculus-1");
  return lessons.map((l) => ({ slug: l.slug }));
}

export default async function CalculusOneLessonPage({ params }: PageProps) {
  const { slug } = await params;
  const lesson = getLessonContent("calculus-1", slug);
  ...
  const adjacent = getAdjacentLessons("calculus-1", slug);
  ...
  return (
    <div className="flex gap-10">
      <article className="prose flex-1 min-w-0">
        <Breadcrumb
          items={[
            { label: "Calculus I", href: "/calculus-1" },
        ...
        {content}
        <LessonNav topicId="calculus-1" prev={adjacent.prev} next={adjacent.next} />
      </article>
      <TableOfContents />
    </div>
  );
