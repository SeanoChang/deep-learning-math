...
export async function generateStaticParams() {
  const lessons = getTopicLessons("calculus-2");
  return lessons.map((l) => ({ slug: l.slug }));
}

export default async function CalculusTwoLessonPage({ params }: PageProps) {
  const { slug } = await params;
  const lesson = getLessonContent("calculus-2", slug);
  ...
  const adjacent = getAdjacentLessons("calculus-2", slug);
  ...
  return (
    <div className="flex gap-10">
      <article className="prose flex-1 min-w-0">
        <Breadcrumb
          items={[
            { label: "Calculus II", href: "/calculus-2" },
        ...
        {content}
        <LessonNav topicId="calculus-2" prev={adjacent.prev} next={adjacent.next} />
      </article>
      <TableOfContents />
    </div>
  );
