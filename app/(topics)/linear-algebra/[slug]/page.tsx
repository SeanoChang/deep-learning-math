...
export async function generateStaticParams() {
  const lessons = getTopicLessons("linear-algebra");
  return lessons.map((l) => ({ slug: l.slug }));
}

export default async function LinearAlgebraLessonPage({ params }: PageProps) {
  const { slug } = await params;
  const lesson = getLessonContent("linear-algebra", slug);
  ...
  const adjacent = getAdjacentLessons("linear-algebra", slug);
  ...
  return (
    <div className="flex gap-10">
      <article className="prose flex-1 min-w-0">
        ...
        {content}
        <LessonNav topicId="linear-algebra" prev={adjacent.prev} next={adjacent.next} />
      </article>
      <TableOfContents />
    </div>
  );
