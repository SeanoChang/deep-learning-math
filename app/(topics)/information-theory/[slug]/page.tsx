...
export default async function InformationTheoryLessonPage({ params }: PageProps) {
  const { slug } = await params;
  const lesson = getLessonContent("information-theory", slug);
  ...
  const adjacent = getAdjacentLessons("information-theory", slug);
  ...
  return (
    <div className="flex gap-10">
      <article className="prose flex-1 min-w-0">
        <Breadcrumb
          items={[
            { label: "Information Theory", href: "/information-theory" },
        ...
        {content}
        <LessonNav topicId="information-theory" prev={adjacent.prev} next={adjacent.next} />
      </article>
      <TableOfContents />
    </div>
  );
