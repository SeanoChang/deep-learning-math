...
export async function generateStaticParams() {
  const lessons = getTopicLessons("optimization");
  return lessons.map((l) => ({ slug: l.slug }));
}

export default async function OptimizationLessonPage({ params }: PageProps) {
  const { slug } = await params;
  const lesson = getLessonContent("optimization", slug);
  ...
  const adjacent = getAdjacentLessons("optimization", slug);
  ...
  return (
    <div className="flex gap-10">
      <article className="prose flex-1 min-w-0">
        ...
        {content}
        <LessonNav topicId="optimization" prev={adjacent.prev} next={adjacent.next} />
      </article>
      <TableOfContents />
    </div>
  );
