...
const SolutionContext = createContext(false);

export function Solution({ children }: { children: ReactNode }) {
  const isRevealed = useContext(SolutionContext);
...
function ProblemRoot({ title, children }: ProblemProps) {
  ...
  return (
    <SolutionContext.Provider value={isRevealed}>
      <div className="my-8 rounded-xl border border-border bg-card shadow-sm">
        ...
        {/* Problem statement + solution (Solution self-hides via context) */}
